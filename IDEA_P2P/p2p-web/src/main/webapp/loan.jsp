	  ...
	  <c:forEach items="${loanInfoList}" var="loanInfo">
		...
		<div class="pro-box <c:if test="${loanInfo.leftProductMoney eq 0}">pro-full</c:if>">
	        ...
	        <div class="rate">${loanInfo.rate}<span>%</span></div>
		      ...
		      <div class="pro-btn">     
				<a href="loan/loanInfo?loanId=${loanInfo.id}" class="btn-1">立即投资</a><!--已满标按钮-->
		      </div>
		    </div>
		  </div>
	  </c:forEach>
              ...
              <tr>
                  <td>
                      ...
                      <c:forEach begin="1" end="${totalPage}" var="page">
                          ...
                          <c:if test="${page != currentPage}">
                              <a href="loan/loan?ptype=${ptype}&currentPage=${page}">${page}</a>
                          </c:if>
                      </c:forEach>
                      ...
                      <c:choose>
                          ...
                          <c:otherwise>
                              <a id="lnkNext" href="loan/loan?ptype=${ptype}&currentPage=1">首页</a>
                              <a id="lnkNext" href="loan/loan?ptype=${ptype}&currentPage=${currentPage-1}">上一页</a>
                          </c:otherwise>
                      </c:choose>
                      ...
                      <c:choose>
                          ...
                          <c:otherwise>
                              <a id="lnkNext" href="loan/loan?ptype=${ptype}&currentPage=${currentPage+1}">下一页 </a>
                              <a id="lnkNext" href="loan/loan?ptype=${ptype}&currentPage=${totalPage}">尾页</a>
                          </c:otherwise>
                      </c:choose>
                  </td>
              </tr>
