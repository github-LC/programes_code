                    ...
                    <td>
                        ...
                        <c:forEach begin="1" end="${totalPage}" var="page">
                            <c:if test="${page == currentPage}">
                                <a style="font-size:20px;color:#1bb2ea">${page}</a>
                            </c:if>
                            <c:if test="${page != currentPage}">
                                <a href="loan/loanInfo?loanId=${loanId}&currentPage=${page}">${page}</a>
                            </c:if>
                        </c:forEach>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a id="lnkNext" href="loan/loanInfo?loanId=${loanId}&currentPage=1">首页</a>
                                <a id="lnkNext" href="loan/loanInfo?loanId=${loanId}&currentPage=${currentPage-1}">上一页</a>
                            </c:otherwise>
                        </c:choose>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a id="lnkNext" href="loan/loanInfo?loanId=${loanId}&currentPage=${currentPage+1}">下一页 </a>
                                <a id="lnkNext" href="loan/loanInfo?loanId=${loanId}&currentPage=${totalPage}">尾页</a>
                            </c:otherwise>
                        </c:choose>
                    </td>
                </tr>
      ...
      <div class="right-calculator" id="huaceng">
        <div class="calculator-cnt">
          ...
          <dl class="usable">
            ...
            <c:otherwise>
	          	<!-- 判断用户是否登录：已登录，显示可用余额 -->
         		<span id="userBalance" style="font-size:18px;color:#ff6161;vertical-align:bottom;">${balance}</span>
            </c:otherwise>
            </c:choose>
          ...
          <div class="invest-btn">
          	<a id="investNow" class="btn-1" onclick="invest();">立即投资</a>
          </div>
...
function checkMoney(){
    ...
    if("" == bidMoney){
        ...
        //计算新手宝的本息
        if(0 == productType){
            money  = bidMoney * (rate/100/365) * cycle;
            money = Math.round(money * Math.pow(10,2)) / Math.pow(10,2);
        }else{
            //计算优选和散标的l本息
            money  = bidMoney * (rate/100/3650) * cycle * 30;
            money = Math.round(money * Math.pow(10,2)) / Math.pow(10,2);
        }
        $(".money").html(money);
    }
...
function invest(){
    ...
    //判断用户是否登陆
    if("" == user){
        if(confirm("你还未登陆，请先登录")){
            window.location.href="login.jsp";
        }
        return false;
    }else if(parseFloat(balance)<parseFloat(bidMoney)){
        $(".max-invest-money").html("你的余额不足，请充值");
        return false;
    }else if(parseFloat(leftProductMoney)<parseFloat(bidMoney)){
        $(".max-invest-money").html("投资金额不能超过产品的剩余可投金额");
        return false;
    }else if(parseFloat(bidMaxLimit)<parseFloat(bidMoney)){
        $(".max-invest-money").html("投资金额不能高于"+bidMaxLimit+"元");
        return false;
    }else if(parseFloat(bidMinLimit)>parseFloat(bidMoney)){
        $(".max-invest-money").html("投资金额不能低于"+bidMinLimit+"元");
        return false;
    }/*else if("" == realName){
        if(confirm("你还未实名认证，请先认证")){
        ...
        $.ajax({
            ...
            error:function(){
                $(".max-invest-money").html("当前投资人数太多，请稍后重试...");
            }
        })
    }
}
